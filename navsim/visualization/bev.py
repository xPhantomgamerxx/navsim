...
def transform_pose(pose_a,pose_b):
    x_a, y_a, theta_a = pose_a
    x_b, y_b, theta_b = pose_b

    x_c = x_a + x_b * cos(theta_a) - y_b * sin(theta_a)
    y_c = y_a + x_b * sin(theta_a) + y_b * cos(theta_a)
    theta_c = theta_a + theta_b  # Add angles

    theta_c = (theta_c + pi) % (2 * pi) - pi

    return [x_c, y_c, theta_c]

def inverse_transform_pose(transform, pose):
    x_t, y_t, theta_t = transform
    x_p, y_p, theta_p = pose

    cos_theta = cos(-theta_t)
    sin_theta = sin(-theta_t)

    x_new = cos_theta * (x_p - x_t) + sin_theta * (y_p - y_t)
    y_new = -sin_theta * (x_p - x_t) + cos_theta * (y_p - y_t)

    theta_new = theta_p - theta_t
...
def add_annotations_to_bev_ax_with_offset(ax: plt.Axes, annotations: Annotations, transform, rel_pose, add_ego: bool = True) -> plt.Axes:
    """
    Adds birds-eye-view visualization of annotations (ie. bounding boxes)
    :param ax: matplotlib ax object
    :param annotations: navsim annotations dataclass
    ...
    for name_value, box_value in zip(annotations.names, annotations.boxes):
        ...
        out = inverse_transform_pose(transform, [x, y, heading])
        x, y, heading = out[0], out[1], out[2]
        box_length, box_width, box_height = box_value[3], box_value[4], box_value[5]
        agent_box = OrientedBox(StateSE2(x, y, heading), box_length, box_width, box_height)
    ...
    if add_ego:
        ego = inverse_transform_pose(rel_pose, [0, 0, 0]) ##################CONTINUE HERE ############################
        car_footprint = CarFootprint.build_from_rear_axle(
            rear_axle_pose=StateSE2(ego[0], ego[1], ego[2]),
            vehicle_parameters=get_pacifica_parameters(),
        )
        add_oriented_box_to_bev_ax(
            ax, car_footprint.oriented_box, AGENT_CONFIG[TrackedObjectType.EGO], add_heading=False
...
def add_trajectory_to_bev_ax_with_offset(ax: plt.Axes, trajectory: Trajectory, transform, config: Dict[str, Any]) -> plt.Axes:
    """
    Add trajectory poses as lint to plot
    :param ax: matplotlib ax object
    :param trajectory: navsim trajectory dataclass
    ...
    transformed_poses = np.array([inverse_transform_pose(transform, pose) for pose in trajectory.poses])    
    poses = np.concatenate([np.array([[0, 0]]), transformed_poses[:, :2]])
    ax.plot(
        poses[:, 1],
        poses[:, 0],
        color=config["line_color"],
...
def add_oriented_box_to_bev_ax(ax: plt.Axes, box: OrientedBox, config: Dict[str, Any], add_heading: bool = True) -> plt.Axes:
    """
