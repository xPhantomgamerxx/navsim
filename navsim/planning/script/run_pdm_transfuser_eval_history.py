...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[Dict[str, Any]]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            improved_score_row["valid"] = False

        if improved_score_row["valid"]:
            ...
            if orig_score_row['score'] > 0 and needed: # needs to be improved
                ...
                improved_pdm_results[-1].update({'result': 'false_negative'})
            elif orig_score_row['score'] == 0 and needed:
                improved_pdm_results[-1].update({'result': 'true_positive'})
        elif improved_score_row["valid"] == False:
            pdm_results.append(orig_score_row)
            improved_pdm_results.append(improved_score_row)
        print("Improved Score ", improved_pdm_results[-1])
        print("")
...
def main(cfg: DictConfig) -> None:
    ...
    num_sucessful_scenarios = pdm_score_df["valid"].sum()
    num_failed_scenarios = len(pdm_score_df) - num_sucessful_scenarios
    average_row = pdm_score_df.drop(columns=["token", "valid", "result"]).mean(skipna=True)
    average_row["token"] = "average"
    average_row["valid"] = pdm_score_df["valid"].all()
    pdm_score_df.loc[len(pdm_score_df)] = average_row
